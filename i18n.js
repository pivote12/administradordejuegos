(function (global) {
  ...
  const GAMES = {
    ...
    "ruleta-especial": {
      ...
      optionsHint: L(
        "Escribí una opción y dale Enter para agregarla. Tocá el color para cambiarlo.",
        "Type an option and press Enter to add it. Tap the color to change it.",
        "Tapez une option et Entrée. Touchez la couleur pour la changer.",
        "Option tippen und Enter. Farbe antippen zum Ändern.",
        "Digite uma opção e Enter. Toque a cor para mudar.",
        "Scrivi un'opzione e Invio. Tocca il colore per cambiarlo."
      ),
      ...
      loadSave: L("Cargar", "Load", "Charger", "Laden", "Carregar", "Carica"),
      deleteSave: L("Borrar guardado", "Delete save", "Supprimer", "Löschen", "Apagar", "Elimina"),
      noSaves: L("No hay ruletas guardadas.", "No saved wheels.", "Aucune roulette enregistrée.", "Keine gespeicherten Räder.", "Nenhuma roleta salva.", "Nessuna ruota salvata."),
      saveMeta: L("{n} opciones · {when}", "{n} options · {when}", "{n} options · {when}", "{n} Optionen · {when}", "{n} opções · {when}", "{n} opzioni · {when}"),
      alertSaveName: L("Escribí un nombre para guardar la ruleta.", "Enter a name to save the wheel.", "Entrez un nom pour enregistrer.", "Gib einen Namen zum Speichern ein.", "Digite um nome para salvar.", "Inserisci un nome per salvare."),
      confirmDeleteSave: L('¿Borrar el guardado "{name}"?', 'Delete save "{name}"?', 'Supprimer « {name} » ?', '„{name}“ löschen?', 'Apagar "{name}"?', 'Eliminare "{name}"?'),
